    ...
    torch::Tensor selected_mask_tensor)
{
    ...
    const auto batch_size = xyz1_tensor.size(0);
    ...
    NoSortKnnLauncher(batch_size, H, W, npoints, kernel_size_H, kernel_size_W, K, flag_copy, distance,
                      stride_h, stride_w, xyz1, xyz2, idx_n2, random_hw, selected_b_idx, selected_h_idx, selected_w_idx, selected_mask, stream);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def(
        "no_sort_knn",
        &torch_NoSortKnnLauncher,
        "torch_NoSortKnnLauncher kernel warpper"
    );
}