...
__global__ void fused_conv_select_k_gpu(int batch_size, int H, int W, int npoints, int kernel_size_H,
										...
										const float *xyz2, const int *idx_n2, const int *idx_fetching, const int *random_hw, long *selected_b_idx, long *selected_h_idx, long *selected_w_idx,
										float *selected_mask, int small_h, int small_w)
{
	...
	int fetched_H_idx = 0, fetched_W_idx = 0;
	...
	idx_fetching += batch_index * npoints * 2;		 // 2d coordinates of central points
	selected_b_idx += batch_index * npoints * K * 1; //(b, npoints, k, 3)，			// batch index of K selected points around central points
	selected_h_idx += batch_index * npoints * K * 1; //(b, npoints, k, 3)，
	selected_w_idx += batch_index * npoints * K * 1; //(b, npoints, k, 3)，


	selected_mask += batch_index * npoints * K * 1; //(b, npoints, k, 1)，坐标有效且距离有效的点，含复制的点（重复使用最近邻的点）

	//////////////      Fused  Conv  Between
	const int MaxPoint = 960;
	for (int current_n = index_thread; current_n < npoints; current_n += stride_thread) //  output_W circle
	{

		int idx_w[MaxPoint], idx_h[MaxPoint];
		float Dist[MaxPoint];
		...
		// NOTE: the following is different
		if (Dist_c <= 1e-10f) //   not  valid  central  points of xyz1
		{
			continue;
		}

		fetched_H_idx = idx_fetching[current_n * 2 + 0];
		fetched_W_idx = idx_fetching[current_n * 2 + 1];
		...
		for (int current_HW_idx = 0; current_HW_idx < kernel_total; ++current_HW_idx) // select points in every kernel element
		{
			...
			int kernel_select_H_idx = fetched_H_idx / stride_h + kernel_HW_idx / kernel_size_W - kernel_half_H; // random select ？？？
			int kernel_select_W_idx = fetched_W_idx / stride_w + kernel_HW_idx % kernel_size_W - kernel_half_W; // random select ？？？
		...
		for (int s_idx = 0; s_idx < K; ++s_idx) // knn
		{
			...
			if ((flag_copy & 0x1) && (s_idx == 0)) // copy the first selected point in xyz2 for K times
			{
...
void FusedConvSelectKLauncher(int batch_size, int H, int W, int npoints, int kernel_size_H,
							  ...
							  const float *xyz1, const float *xyz2, const int *idx_n2, const int *idx_fetching, const int *random_hw,
							  ...
							  float *selected_mask, int small_h, int small_w, cudaStream_t stream)
{
	...
	fused_conv_select_k_gpu<<<batch_size, 1024, 0, stream>>>(batch_size, H, W, npoints, kernel_size_H, kernel_size_W, K, flag_copy, distance, stride_h, stride_w, xyz1, xyz2, idx_n2, idx_fetching, random_hw, selected_b_idx, selected_h_idx, selected_w_idx, selected_mask, small_h, small_w);
